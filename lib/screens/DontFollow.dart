...
class DontFollowPage extends StatefulWidget {
  final List<User>? nonFollowersFromFile;

  DontFollowPage({this.nonFollowersFromFile});

  @override
  _DontFollowPageState createState() => _DontFollowPageState();
}

class _DontFollowPageState extends State<DontFollowPage> with SingleTickerProviderStateMixin {
  int totalNonFollowers = 0;
  ...
  Future<void> _initializeDatabaseAndLoadData() async {
    ...
    if (widget.nonFollowersFromFile != null) {
      CountNonFollowers();
      loadUsers();
    } else {
  ...
  Future<void> loadUsers() async {
    List<User> loadedUsers = await dbHelper.getNonFollowers();
    if (mounted) {
      setState(() {
        users = loadedUsers;
      });
  ...
  Future<void> CountNonFollowers() async {
    List<User> loadedUsers = await dbHelper.getNonFollowers();
    if (mounted) {
      setState(() {
        users = loadedUsers;
        totalNonFollowers = users.length;
      });
    }
  }
  ...
  Future<void> _deleteUser(int? userId) async {
    if (userId != null) {
      ...
      await dbHelper.deleteNonFollower(userId);
      CountNonFollowers();
      await loadUsers();
    } else {
  ...
  void _toggleFavorite(int index) {
    setState(() {
      ...
      dbHelper.updateNonFollowerFavoriteStatus(users[index].id, users[index].isFavorite);
    });
  }

  void _refreshNonFollowers() {
    CountNonFollowers();
    loadUsers();
  }
